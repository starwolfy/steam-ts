...
exports.launch = function() {
    ...
    steamClient.on('error', function () {
        if (twoFactor = configjson.twoFactor.enabled) {
            console.log("Either Steam is down or you entered the wrong authentication password.");
        } else {
            console.log("Steam went down.");
        }
    });
        ...
        if (message != "") {
            ...
            if (message == '!verify') {
                ...
                if (typeof verifying[source] === "undefined") {
                    ...
                    console.log("Executing checkKey with third param: " + editdescription);
                    checkKey(message, source, editdescription);
                }
            }
        }
    });

    var cl = new TeamSpeakClient(ts_ip, q_port);
    exports.cl = cl;

    //log into the teamspeak query server and use a virtual machine so we can dig deeper for information
    function loginTs(q_username, q_password, q_vserverid) {
        cl.send("login", {client_login_name: q_username, client_login_password: q_password}, function (err) {
            if (typeof err !== "undefined") {
                ...
                cl.send("use", {sid: q_vserverid}, function (err) {
                    if (typeof err !== "undefined") {
                        console.log(err);
                    } else {
                        console.log("Using virtual server " + q_vserverid + " now.");
                        launchChannels();
    ...
    loginTs(q_username, q_password, q_vserverid);
                        ...
                        cl.send("clientinfo", {clid: user_clid}, function (err, response) {
                            if (typeof err !== "undefined") {
                                ...
                                if (response.client_servergroups == defaultrankid) {
                                    ...
                                    console.log("\nThe given TeamSpeak account has already been approved.");
            ...
            cl.send("sendtextmessage", {targetmode: 1, target: clid, msg: sendmessage}, function (err) {
                if (typeof err !== "undefined") {
                    ...
                    steamFriends.sendMessage(source, '\nWe have messaged you on TeamSpeak.' +
                        '\nPlease copy the random token and paste it here in the Steam chat:', Steam.EChatEntryType.ChatMsg);
    ...
    function checkKey(message, source, editdescription) {
        if (exports.teamspeakstatus) {
            ...
            for (i=0;i<keysArray.length;i++) {
                if (keysArray[i] == message) {
                    ...
                    var current_clid = verifying[source];

                    cl.send("clientinfo", {clid: current_clid}, function (err, response) {
                        if (typeof err !== "undefined") {
                            console.log(err);
                        } else {
                            ...
                            if (verified_users == "") {
                                console.log("Tried to write an empty var to verified.json...");
                                steamFriends.sendMessage(source, '\nInternal error, please try again.', Steam.EChatEntryType.ChatMsg);
                                return;
                            }
                            ...
                            cl.send("servergroupaddclient", {sgid: wantedrankid, cldbid: cldbid}, function (err) {
                                if (typeof err !== "undefined") {
                                    ...
                                    if (editdescription) {
                                        cl.send("clientedit", {clid: current_clid, client_description: source}, function (err) {
                                            if (typeof err !== "undefined") {
                                                console.log(err);
                                            } else {
