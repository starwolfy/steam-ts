...
module.exports = function(token, senderID, currentlyVerifying, steamClient, teamspeakClient, config) {
    ...
    async.waterfall([
        ...
        function(current_clid, callback) {
        ...
        function(response, current_clid, callback) {
            ...
            teamspeakClient.send("servergroupaddclient", {sgid: config.wantedrankid, cldbid: cldbid}, function (err) {
                if (typeof err !== "undefined") {
                    ...
                    pushMe.steamid = steamid3;
                    pushMe.teamspeakid = identity;
                    pushMe.nicknames = [response.client_nickname];
                    pushMe.addresses = [response.connection_client_ip];
                    ...
                    callback(null, config.editdescription, current_clid, response);
                }
            })
        }, function(editdescription, current_clid, response, callback) {
